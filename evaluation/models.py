...
class qcGEM(nn.Module):
    ...
    def get_embedding(self, batch_data, freeze: bool = True, layer_idx: int = -1):
        """
        Per-graph embedding
        """
        if freeze:
            self.pre_process.eval()
            self.encoder.eval()
            with torch.no_grad():
                batch_data_processed, _ = self.pre_process(batch_data)
                batch_data_processed, Encoder_Emebdding_Dict = self.encoder(batch_data_processed)
                return self.encoder.pool_graph_embedding(batch_data_processed, Encoder_Emebdding_Dict, layer_idx=layer_idx)

        batch_data_processed, _ = self.pre_process(batch_data)
        batch_data_processed, Encoder_Emebdding_Dict = self.encoder(batch_data_processed)
        return self.encoder.pool_graph_embedding(batch_data_processed, Encoder_Emebdding_Dict, layer_idx=layer_idx)

    def get_disentangled_embedding(self, batch_data):
        """
        [Ablation / analysis only]
        """
        self.pre_process.eval()
        self.encoder.eval()
        with torch.no_grad():
            batch_data_processed, _ = self.pre_process(batch_data)
            batch_data_processed, Encoder_Emebdding_Dict = self.encoder(batch_data_processed)
            return self.encoder.pool_disentangled_components(batch_data_processed, Encoder_Emebdding_Dict)
...
class qcGEM_Encoder(nn.Module):
    def __init__(self, 
        ...
        self._embed_pool = aggr.MeanAggregation()

    def pool_graph_embedding(self, batch_data_processed, Encoder_Emebdding_Dict, layer_idx: int = -1):
        """
        Per-graph readout
        """
        ...
        global_features = Encoder_Emebdding_Dict['Global'][layer_idx]
        node_features = Encoder_Emebdding_Dict['Node'][layer_idx]
        edge_features = Encoder_Emebdding_Dict['Edge'][layer_idx]

        node_pooled = self._embed_pool(x=node_features, index=batch)
        edge_pooled = self._embed_pool(x=edge_features, index=batch[edge_index[0]])

        graph_repr = torch.concat([global_features, node_pooled, edge_pooled], dim=-1)
        graph_repr = F.layer_norm(graph_repr, (graph_repr.size(-1),))
        return graph_repr

    def pool_disentangled_components(self, batch_data_processed, Encoder_Emebdding_Dict):
        """
        [Ablation / analysis only]
        """
        global_all = [g.clone() for g in Encoder_Emebdding_Dict['Global_AllLayers']]
        node_all = list(Encoder_Emebdding_Dict['Node_AllLayers'])
        edge_all = list(Encoder_Emebdding_Dict['Edge_AllLayers'])

        global_all.append(batch_data_processed['global_features'].detach().clone())
        node_all.append(batch_data_processed['node_features'].detach().clone())
        edge_all.append(batch_data_processed['edge_features'].detach().clone())

        return {'global': global_all, 'node': node_all, 'edge': edge_all}
    ...
    def forward(self, batch_data_processed):        
        ...
        Node_PE_Stru_ = self.GM_2(pos, atom_type, batch)
        Node_PE_Cen = None 
        ...
        Encoder_Emebdding_Dict['Global_AllLayers'] = GF_State_Embedding
        Encoder_Emebdding_Dict['Node_AllLayers'] = Node_State_Embedding
        Encoder_Emebdding_Dict['Edge_AllLayers'] = Edge_State_Embedding
...
from torch_geometric.nn import norm, aggr

class ADMET_probe(nn.Module):
    def __init__(self, qcGEM_model = None,
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None,
                 device = None, task_name = None):
        super(ADMET_probe, self).__init__()

        assert task_name is not None
        assert qcGEM_model is not None
        ...
        self.qcGEM_model = qcGEM_model

        self.predict_input_proj = nn.Sequential(
            nn.Linear(self.global_emb_dim + self.node_emb_dim + self.edge_emb_dim, self.node_emb_dim * 2),
            self.act_fn)

        self.predict_body_MLP = nn.ModuleList()
        self.predict_body_MLP.append(nn.Dropout(p=0.3))
        self.predict_body_MLP.append(nn.Linear(self.node_emb_dim * 2, self.node_emb_dim * 1))
        self.predict_body_MLP.append(self.act_fn)

        if self.task_name in ['ESOL', 'FreeSolv', 'Lipophilicity', 'Bioavailability', 'Caco2', 'Solubility_AqSolDB','PPBR_human', 'PPBR_OtherSpecies', 'VDss', 'ClearanceHepatocyte', 'ClearanceMicrosome', 'HalfLife', 'LD50']:
            self.predict_head = nn.Sequential(nn.Linear(self.node_emb_dim * 1, int(self.node_emb_dim / 2)), self.act_fn, nn.Linear(int(self.node_emb_dim / 2), 1))
        elif self.task_name in ['BBBP', 'BACE', 'HIA', 'PAMPA', 'Pgp','BBB', 'CYP1A2_Veith', 'CYP2C19_Veith', 'CYP2C9_Substrate', 'CYP2C9_Veith', 'CYP2D6_Substrate', 'CYP2D6_Veith', 'CYP3A4_Substrate', 'CYP3A4_Veith', 'Ames', 'Carcinogens', 'DILI', 'hERG_Blockers', 'SkinReac']:
            self.predict_head = nn.Sequential(nn.Linear(self.node_emb_dim * 1, int(self.node_emb_dim / 2)), self.act_fn, nn.Linear(int(self.node_emb_dim / 2), 2))
        else:
            print('Type in a right task name!')

        self.to(self.device)

    def forward(self, batch_data):
        golbal_F = self.qcGEM_model.get_embedding(batch_data, freeze=True)

        golbal_F = self.predict_input_proj(golbal_F)
        for num, mlp in enumerate(self.predict_body_MLP):
            golbal_F = mlp(golbal_F)
        golbal_F = self.predict_head(golbal_F)

        return golbal_F


class Cliff_probe(nn.Module):
    def __init__(self, qcGEM_model = None,
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None,
                 device = None, task_name = None):
        super(Cliff_probe, self).__init__()
...
class Cliff_probe_finetune(nn.Module):
    def __init__(self, qcGEM_model = None,
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None,
                 device = None, task_name = None):
        super(Cliff_probe_finetune, self).__init__()
    ...
    def forward(self, batch_data):
        golbal_F = self.qcGEM_model.get_embedding(batch_data, freeze=False)
...
class PLI_opioids_probe(nn.Module):
    def __init__(self, qcGEM_model = None,
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None,
                 device = None, task_name = None):
        super(PLI_opioids_probe, self).__init__()
...
class PLI_probe(nn.Module):
    def __init__(self, qcGEM_model = None,
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None,
                 protein_input_dim = None, protein_output_dim = None,
                 device = None, task_name = None):
        super(PLI_probe, self).__init__()

        assert task_name is not None
        assert qcGEM_model is not None
        assert protein_input_dim is not None and protein_output_dim is not None
        ...
        self.Protein_nn = nn.Linear(self.protein_input_dim, self.protein_output_dim)

        self.predict_input_proj = nn.Sequential(
            nn.Linear(self.protein_output_dim + self.global_emb_dim + self.node_emb_dim + self.edge_emb_dim, self.node_emb_dim * 3),
            self.act_fn)

        self.predict_body_MLP = nn.ModuleList()
        self.predict_body_MLP.append(nn.Dropout(p=0.3))
        self.predict_body_MLP.append(nn.Linear(self.node_emb_dim * 3, self.node_emb_dim * 2))
        self.predict_body_MLP.append(self.act_fn)
        self.predict_body_MLP.append(nn.Linear(self.node_emb_dim * 2, self.node_emb_dim * 1))
        self.predict_body_MLP.append(self.act_fn)
        self.predict_head = nn.Sequential(nn.Linear(self.node_emb_dim * 1, int(self.node_emb_dim / 2)), self.act_fn, nn.Linear(int(self.node_emb_dim / 2), 1))
    ...
    def forward(self, batch_data):
        protein_batch = batch_data[0]
        ligand_batch = batch_data[1]

        ligand_F = self.qcGEM_model.get_embedding(ligand_batch, freeze=True)
        protein_F = self.act_fn(self.Protein_nn(protein_batch.global_protein_emb))

        golbal_F = torch.concat([protein_F, ligand_F], dim=-1)
