...
class ADMET_probe(nn.Module):
    def __init__(self, Data_pre_processor = None, EmbGenerator = None, EmbeddingMode = None, 
                 global_emb_dim = None, node_emb_dim = None, edge_emb_dim = None, remove_self_loop = None,
                 device = None, task_name = None, norm = None):
        ...
        self.EmbGenerator = EmbGenerator
        
        self.predict_body_ReadOut_Node = aggr.MeanAggregation()
        self.predict_body_ReadOut_Edge = aggr.MeanAggregation()
        self.NormPreProcess = nn.LayerNorm(128 * 3, elementwise_affine = False)

        self.predict_ReadOut_DownDim = nn.Sequential(nn.Linear(self.node_emb_dim + self.edge_emb_dim + self.global_emb_dim, self.node_emb_dim * 2), self.act_fn)
    ...
    def forward(self, batch_data):
        ...
        if self.EmbeddingMode == 'Hard':
            self.Data_pre_processor.eval()
            self.EmbGenerator.eval()
            with torch.no_grad():
                batch_data, batch_data_label = self.Data_pre_processor(batch_data)
                batch_data, Encoder_Emebdding_Dict = self.EmbGenerator(batch_data)
                edge_index = batch_data['edge_index']
                global_features = Encoder_Emebdding_Dict['Global']
                node_features = Encoder_Emebdding_Dict['Node']
                edge_features = Encoder_Emebdding_Dict['Edge']
        elif self.EmbeddingMode == 'Soft':
            batch_data, batch_data_label = self.Data_pre_processor(batch_data)
            batch_data, Encoder_Emebdding_Dict = self.EmbGenerator(batch_data)
            edge_index = batch_data['edge_index']
            global_features = Encoder_Emebdding_Dict['Global']
        ...
        golbal_F_Node = self.predict_body_ReadOut_Node(x = node_features, index = batch)
        golbal_F_Edge = self.predict_body_ReadOut_Edge(x = edge_features, index = batch[batch_data['edge_index'][0]])
        golbal_F = torch.concat([global_features, golbal_F_Node, golbal_F_Edge], dim = -1)
        golbal_F = self.NormPreProcess(golbal_F)

        golbal_F = self.predict_ReadOut_DownDim(golbal_F)
