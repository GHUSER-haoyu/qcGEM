    ...
    def reset_parameters(self):
        if self.in_channels > 0:
            if self.weight_initializer == 'glorot':
                ...
                bound = 1.0 / (math.sqrt(self.weight.size(-1) + 1e-10) + 1e-10)
...
def spherical_bessel_formulas(n):
    ...
    j = [sym.sin(x) / (x + 1e-10)]  # j_0
    a = sym.sin(x) / (x + 1e-10)
    for i in range(1, n):
        b = sym.diff(a, x) / (x + 1e-10)
...
def bessel_basis(n, k):
    ...
    for order in range(n):
        ...
        normalizer_tmp = (
            1 / ((np.array(normalizer_tmp) + 1e-10) ** 0.5 + 1e-10 )
        ...
        if zero_m_only:
            ...
            for l in range(1, L):
                P_l_m[l][l] = sym.simplify(
                    (1 - 2 * l) * ((1 - z ** 2) + 1e-10) ** 0.5 * P_l_m[l - 1][l - 1]
                )  # P_00, P_11, P_22, P_33
...
class GeometryModule_(nn.Module):
    def __init__(
            self,
            cutoff = None,
            num_layers = None,
        ...
        super(GeometryModule_, self).__init__()
        self.out_channels = out_channels
        self.cutoff = cutoff
        ...
        self.feature_expansion_1 = torsion_emb(num_radial=num_radial, num_spherical=num_spherical, cutoff=cutoff)
        self.feature_expansion_2 = angle_emb(num_radial=num_radial, num_spherical=num_spherical, cutoff=cutoff)
    ...
    def _forward(self, pos, z, batch):
        num_nodes = z.size(0)

        edge_index_inner = radius_graph(pos, r=self.cutoff, batch=batch)
        j, i = edge_index_inner
        ...
        # Calculate torsions.
        dist_ji = (pos_ji.pow(2).sum(dim=-1) + 1e-10).sqrt()
        plane1 = torch.cross(-pos_ji, pos_in0)
        plane2 = torch.cross(-pos_ji, pos_in1)
        a = (plane1 * plane2).sum(dim=-1)  # cos_angle * |plane1| * |plane2|
        b = (torch.cross(plane1, plane2) * pos_ji).sum(dim=-1) / (dist_ji + 1e-10)
        phi = torch.atan2(b, a)
        ...
        feature1 = self.feature_expansion_1(dist, theta, phi)
        feature2 = self.feature_expansion_2(dist, tau)

        # Interaction blocks.
        for interaction_block in self.interaction_blocks:
            x = interaction_block(x, feature1, feature2, edge_index_inner, batch)
